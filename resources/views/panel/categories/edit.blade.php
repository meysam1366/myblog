                    ...
                    <div>
                        ...
                        <select type="text" id="parent_id" name="parent_id" class="form-input" autocomplete="off">
                            ...
                            @foreach($categories as $category_parent)
                                <option value="{{ $category_parent->id }}" {{ $category->parent_id == $category_parent->id ? 'selected' : '' }}>{{ $category_parent->title }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label for="title" class="form-label">Title *</label>
                        <input type="text" id="title" name="title" value="{{ old('title', $category->title) }}" placeholder="Title" class="form-input" autocomplete="off" required>
                    </div>
                    <div>
                        ...
                        <select type="text" id="language_id" name="language_id" class="form-input" autocomplete="off" required>
                            ...
                            @foreach($languages as $language)
                                <option value="{{ $language->id }}" {{ $category->language_id == $language->id ? 'selected' : '' }}>{{ $language->title }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="flex items-center gap-2 mt-5">
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="checkbox" {{ $category->status == 1 ? 'checked' : '' }} value="1" class="appearance-none peer" name="status">
